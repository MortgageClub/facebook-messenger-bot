      ...
      if (error) {
        ...
        if(utils.isDefined(response.body) && utils.isDefined(response.body.speech)) {
          ...
          if(response.body.speech.status_code == 200 ){
            ...
            fbServices.sendFBMessage(sender, fbServices.textMessage("Address : " + response.body.speech.address +  "Lower rate refinance : New interest rate" + response.body.speech.lower_rate_refinance.new_interest_rate + " New monthly payment: " + response.body.speech.lower_rate_refinance.new_monthly_payment ));
